...
public class EnvWss<T> extends network.Client {
    ...
    public EnvWss(String addr, int port, Mode mode, int numOfTasks, boolean[][][] bin_b, boolean[][] Ws_b, boolean[] threshold_b, int len_a, ArrayList<String> PartyB_IDs, boolean[][] mask) {
        this.addr = addr;
        this.port = port;
        this.mode = mode;
        this.bin_b = bin_b;
    ...
    public void implement() {
        ...
        try {
            ...
            for (int i = 0; i < this.numOfTasks; i++) {
                ...
                if(mask!=null){
                    tmp0 = new PatientLinkage4GadgetWsInputs<>(Arrays.copyOfRange(bin_a, Range0[i][0], Range0[i][1]), Ws_a, threshold_a, eva, "Alice", i, Arrays.copyOfRange(mask, Range0[i][0], Range0[i][1]));
                }else{
                    tmp0 = new PatientLinkage4GadgetWsInputs<>(Arrays.copyOfRange(bin_a, Range0[i][0], Range0[i][1]), Ws_a, threshold_a, eva, "Alice", i, null);
                }
                PatientLinkage4GadgetWsInputs<T> tmp1 = new PatientLinkage4GadgetWsInputs<>(this.bin_b, Ws_b, threshold_b, eva, "Bob", i, mask);
